<script>
    ...
    async function loadData() {
        try {
            ...
            if (data.length > 0) {
                currentYear.set(data[0].year);
                currentCount.set(data[0].count);
                dataLoaded = true;
            } else {
    ...
    function updateScales() {
        ...
        const maxCount = Math.max(
            50,
            d3.max(currentData, (d) => d.count) * 1.1
        );
    ...
    function updateMarkers() {
        const visibleMarkers = historicalMarkers.filter((marker) => {
            return (
                marker.showAt <= $currentYear &&
                $currentYear - marker.showAt <= 15
            ); // Fade after 15 years
        });
        ...
        enterGroups.transition().delay(200).duration(400).style("opacity", 1);
    }
    ...
    function updateChart() {
        ...
        dots.selectAll(".dot").transition().duration(100).style("opacity", 0);
        ...
        path.datum(currentData).transition().duration(300).attr("d", line);

        setTimeout(() => {
            const dotSelection = dots
                .selectAll(".dot")
            ...
            dotSelection
                ...
                .style("fill", (d) =>
                    d.year === $currentYear ? "#FF5722" : "#2196F3"
                )
                .style("stroke", "white")
                .style("stroke-width", 2)
                .style("opacity", 0);
        ...
        svg.select(".title").text(`UFO Sightings: ${$currentYear}`);

        svg.select(".current-year").text(`${$currentCount} sightings reported`);
    }
    ...
    function setupScrollListener() {
        ...
        const handleScroll = (event) => {
            ...
            if (event.deltaY > 0 && $currentYearIndex < data.length - 1) {
                currentYearIndex.set($currentYearIndex + 1);
            } else if (event.deltaY < 0 && $currentYearIndex > 0) {
                currentYearIndex.set($currentYearIndex - 1);
            }

            if (oldIndex !== $currentYearIndex) {
                currentYear.set(data[$currentYearIndex].year);
                currentCount.set(data[$currentYearIndex].count);
                updateChart();
            }
    ...
    function jumpToEnd() {
        currentYearIndex.set(data.length - 1);
        currentYear.set(data[$currentYearIndex].year);
        currentCount.set(data[$currentYearIndex].count);
        updateChart();
    }
...
<div class="chart-wrapper">
    <div class="controls">
        <div class="control-buttons">
            <button on:click={resetToStart} class="control-btn"
                >Start (1940)</button
            >
        ...
        <div class="progress">
            ...
            <div class="progress-bar">
                <div
                    class="progress-fill"
                    style="width: {($currentYearIndex / (data.length - 1)) *
                        100}%"
                ></div>
            </div>
        </div>
